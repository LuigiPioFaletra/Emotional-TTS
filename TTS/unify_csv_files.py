...
def generate_metadata(audio_folder, csv_files, output_file="metadata.csv"):
    """
    Genera un file 'metadata.csv' unificato a partire da una lista di CSV contenenti informazioni sui file audio, verificando la presenza fisica degli audio e la presenza di trascrizioni con token emozionali.
    """
    out_lines = []
    missing_audio = []
    ...
    for idx, row in combined_df.iterrows():
        ...
        token_text = row.get("transcription_with_token", "")
        if pd.isna(file_name) or not str(file_name).strip():
            continue
        if pd.isna(token_text) or not str(token_text).strip():
...
generate_metadata(audio_folder="./Dataset completo", csv_files=csv_sorgenti)
