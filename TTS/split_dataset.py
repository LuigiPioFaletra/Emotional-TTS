...
def prepare_dataset_with_split(csv_input, csv_output, val_per_class=220, test_per_class=220, seed=3407):
    """
    Prepara il dataset finale aggiungendo lo split train/validation/test, riorganizzando le colonne
    e verificando i conteggi per split ed emozione.
    """
    ...
    for emo, group in df.groupby("emotion"):
        ...
        if len(indices) < val_per_class + test_per_class:
            raise ValueError(f"La classe '{emo}' ha solo {len(indices)} esempi, servono almeno {val_per_class + test_per_class}.")
